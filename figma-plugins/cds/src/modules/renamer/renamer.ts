...
export function analyzeProductDesign(): RenamerResult {
  ...
  for (var node of walkTree(targets)) {
    ...
    if (isInsideCDSInstance(node)) { skipped++; continue; }
...
// ============================================
// Mode 2: CDS 라이브러리 리네이밍 + 프로퍼티 점검
// ============================================

export function analyzeCDSLibrary(): RenamerResult {
...
function isInsideCDSInstance(node: SceneNode): boolean {
  var current = node.parent;
  while (current && current.type !== 'PAGE') {
    if (current.type === 'INSTANCE') {
      return isCDSInstance(current as InstanceNode);
    }
