...
### Changed
- **Badge final PSD 원본 갱신 기록** — 사용자가 의도적으로 수정한 `assets/Badge/final.psd`를 최신 badge final source로 기록했다. 파일은 PSD 포맷 `986×1024`, 약 29MB이며 SHA-256은 `2c1caee7bced464c9022ffa179e148c843c5da2c16a90c9d1f05335d0ff71ad5`다. 기존 badge reference/source 정리 흐름을 유지하며, `agent-center`의 pending record 커밋은 별도 세션 진행 중이라 이번 CDS record 범위에서 제외했다.
- **Badge frame/artwork production strategy 정리** — 배지 프레임이 RPG 메달처럼 읽히는 문제를 확인하고, 기존 자동화 논의를 `frame-locked sandwich workflow`로 정리했다. 프레임은 최종 합성에서 재생성하지 않고 고정 자산으로 보호하며, GPT Image 2는 통합 시안/아트웍 방향과 프레임 상호작용을 만드는 용도로만 사용한다는 원칙을 문서화했다. 단계별 베스트 합성 레퍼런스, mask/occlusion/overflow 기준, palette 기준이 자동화 전에 필요하다는 결론을 남겼고, 새 프레임 설계가 필요하다는 사용자 판단에 따라 기존 RPG풍 프레임 자동화는 중단 상태로 기록했다.
- **CDS naming/token completion gate 강화** — Feed Detail 작업에서 발견된 네이밍/토큰 미적용 재발을 막기 위해 CompletionEvidence에 `namingGate`를 추가하고 `runCompletionGate()`가 hard naming failure를 record/final handoff 전에 차단하도록 보강. `naming-policy.md` v2.0 Section 3-8을 `figma-plugins/cds/src/modules/renamer/rules.ts`의 executable `NamingRule`로 매핑하고, layer slash 금지/variant slash 허용, Lucide icon 강제, M3 `Container` evidence requirement, naming exception owner/approver/review/expiry 계약을 추가. Token binding gate는 CDS typography token style(`text-*/leading-*`)과 CDS `Mode` 컬렉션 색상 변수 binding을 요구하고, color value match만으로는 PASS하지 않도록 조정. Migrator는 hardcoded/remote/non-CDS color/text style을 CDS Mode/token style로 재바인딩하는 방향으로 강화. Contract tests 22개 PASS, `npm run typecheck`, `npm run build` PASS.
- **Profile Action Button 공통화 + Feed author action 정리** — `Follow Button` component set(`21638:4169`)을 common composed component `Profile Action Button`으로 승격하고 `Components > Composed > Profile Actions > Main content`로 이동. 기존 `Status=Follow / Follow Back / Following / Invite` variant는 유지하되 description을 people-context relationship/action CTA 용도로 정리. `Feed Card` header의 raw follow button을 `Profile Action Button` 인스턴스로 교체하고 `↳ Show Author Action` BOOLEAN + `↳ Author Action` INSTANCE_SWAP property로 노출해 내 게시물에서는 author action만 숨기고 ellipsis menu는 유지하도록 보정. 임시 인스턴스 검증에서 hide, `Following`, `Invite` swap PASS.
- **Composed child Show property `↳` 네이밍 정리** — 사용자 지적에 따라 `Invite Composed`(`20358:7313`), `Feed Cards`(`21721:6809`), `Lounge Cards`(`20726:2860`) 세 그룹을 스캔하고 하위 요소를 제어하는 `Show*` boolean property 36개에 `↳` prefix를 적용. paired child property도 필요한 경우 함께 정리(`Name`/`Like Count`/`Reply Count`/`Right Slot`/`Price` 등). `.Backup` 계열과 `Feed Card` 최상위 섹션 토글은 제외. 13개 affected component owner에서 임시 인스턴스로 `↳ Show* = false` set 검증 PASS, 잔여 누락 0건. Figma Plugin API에 비파괴 property reorder가 없어 순서 재배치는 override reset 위험 때문에 미적용.
- **Feed Screen published CDS use-site 원본 속성 복원** — 사용자 CDS publish 후 제품 파일 `2026-05` target `Feed Screen`(`CS2ZhrORl4E1szQfTe2UvO/24112:14935`)의 6개 published `Feed Card` 인스턴스(`28452:1811`, `2457`, `2749`, `3056`, `3356`, `3659`)를 backup 원본 `28453:33210`의 local frame 6개(`28453:33236`, `33293`, `33344`, `33395`, `33445`, `33497`)와 순서 매핑해 속성 이전. Header name/date/button state, avatar/creator badge image, `Feed Lounge Strip` title/attendee/challenge thumbnail/avatar group, media 인증이미지(원본에 image fill이 있는 카드 3~6), Reaction count, Addon Footer actor/count/status 조사 suffix(`이/가`), Comment text를 원본 기준으로 적용. 후속 지적 "인증 이미지는 비어있어" 확인 후 target `Feed Content Section` frame에 원본 image paint를 명시 재주입: 카드 3 `9dfe...`, 카드 4 `da2b...`, 카드 5 `2584...`, 카드 6 `1451...`; 카드 1~2는 원본에도 대형 인증 image fill 없음. Postflight key-field compare 6/6 PASS, mismatch 0.
- **CDS Feed Cards probe cleanup** — 사용자 지적에 따라 publish-facing `Components > Composed > Feed Cards > Main content`(`21721:6812`)에 남아 있던 QA `[PROBE]` 인스턴스 10개를 전수 점검 후 삭제. 삭제 대상은 direct child `INSTANCE` + name prefix `[PROBE]` + reactions 0으로 guard했고, CDS 파일 전체 `[PROBE]` 잔존 0개 확인. `Reaction Bar`/`Comment Item`/`Feed Addon Footer`/`Feed Card`/`Feed Lounge Strip` component definitions와 property contract는 변경 없이 PASS. 산출물: `.ai/pipeline/runs/20260508-154755_cds-probe-cleanup/`.
- **Feed Card visual parity Option B CDS-side 완료** — Claude handoff 이슈를 `/goal -play -director`로 이어받아 기존 `Lounge Card` height drift 대신 신규 composed `Feed Lounge Strip`(`21743:9854`, key `4ea818eba7746173a2c69f5f8df3f13fda116ca3`, 375×96)을 생성하고 `Feed Card` master(`21732:3062`)의 `Lounge Card Slot` default를 교체. root fill/text style token binding, parent-scoped property matrix, layout contract, instance override/responsive/long text/bounds probes PASS. Header Slot ellipsis override는 nested instance traversal 실패로 제품 use-site 후속 처리로 기록. 제품 파일 `2026-05` 6개 use site 반영은 CDS publish + library update 후 진행 필요. 산출물: `.ai/pipeline/runs/20260508-150832_feed-card-visual-parity-option-b/`, 리뷰: `reviews/2026-05-08_feed-card-visual-parity-option-b.md`.
- **Feed Screen 6 use site → 신규 Feed Card 인스턴스 교체 완료** — 사용자 publish 후 use site replacement 진행. 제품 파일 `2026-05`에서 Feed Card componentKey `1a348920b824461793300098c74f832f20f758b7` import 정상(10 props 노출). 6 source feed frames(`24112:14976`, `24112:15039`, `24112:15087`, `24112:15135`, `24112:15182`, `24112:15231`) 각각 preserve-source-overrides procedure로 처리: (a) Profile Card name/date/avatar fills 추출 + 적용, (b) Lounge Card title text + Challenge Thumbnail image fill 추출 + 적용, (c) Reaction Bar Like/Reply count 적용, (d) Feed Addon Footer Actor/Count/Status text 추출(suffix strip 휴리스틱) + 적용, (e) Comment Item 1·2 name/description 적용 + Right Slot variant swap (icon heart vs text "더보기"). Source frames는 신규 PAGE `_Archive Feed Screen 2026-05-08`(`28452:1633`)으로 이동(Auto Layout escape 안전). 6/6 PASS, errors 0. 알려진 이슈: Lounge Card 자연 height 196 vs 원본 LOCAL frame 96의 시각 drift 가능성 (사용자 검증). 산출물: `.ai/pipeline/runs/20260508-144125_feed-screen-use-site-replacement/`, after 스크린샷 `exports/2026-05-08_feed-cards-cds/feed-screen-after-replacement.png`.

### Changed
- **Feed Cards 기본값 보정 + INSTANCE_SWAP 한계 해소** — 사용자 피드백 "댓글 더보기 버튼이 텍스트가 아니라 아이콘 버튼으로 적용됨" + "기존 CDS 콤포넌트 대체 점검" 반영. (1) Comment Item Right Slot default를 Type=Default Size=Icon-Small (32×32 icon-only)에서 **Type=Ghost Size=Small Label="더보기"**로 변경. 원본 Feed Screen 첫 카드 시각과 일치. (2) `addComponentProperty INSTANCE_SWAP` 호환 이슈 해결: 5가지 default value 형식 테스트 결과 **`instance.mainComponent.id`** 만 PASS (Figma plugin API 문서는 "key"라 표기하지만 실제 동작은 node id). Comment Item에 `Right Slot` INSTANCE_SWAP property 추가 (4→5 props), Feed Card에 6개 slot props (`Header Slot`/`Lounge Card Slot`/`Reaction Bar Slot`/`Addon Footer Slot`/`Comment Slot 1`/`Comment Slot 2`) INSTANCE_SWAP 추가 (4→10 props). 모든 nested instance를 새 prop에 mainComponent + visible refs 재바인딩. (3) 컴포넌트 4종 유지 결정 (사용자 원칙 "동일 구조만 대체, 변형 폭발 X" 적용): CDS `Item` ComponentSet (vertical 2-line + Slot Left)은 Comment Item (horizontal Name+Desc no Slot Left)와 구조 다르므로 extend 시 6→9 variant 폭발 위험. 별도 컴포넌트 유지 결정. 산출물: `.ai/pipeline/runs/20260508-143314_feed-cards-default-button-fix/`, before `exports/2026-05-08_feed-cards-cds/feed-cards-group.png` / after `feed-cards-group-after-fix.png`.

### Added
- **Feed Cards CDS 컴포넌트 4종 + 그룹 신설** — Feed Screen(`CS2ZhrORl4E1szQfTe2UvO/24112:14935`)에서 신규 CDS 컴포넌트 후보를 도출 후 `Components > Composed > Feed Cards > Main content`(`H36eNEd6o7ZTv4R7VcyLf2/21721:6812`)에 4종 생성. **Reaction Bar**(`21723:2908` / key `1065b60af9b20c02d5c47a80e1e4b3475d368e5c`, 375×38, Heart Outline+Filled overlay 모델로 `Is Liked` BOOLEAN 구현, Phosphor `chat-centered-dots` → Lucide `message-circle-more` 정합), **Comment Item**(`21725:2939` / key `f2d39007910743ce240c9f8591ab766d2a9dcec3`, 343×32), **Feed Addon Footer**(`21726:2953` / key `3a4eb5a7dac4d1234cc5ee13e13aebcfd2e8d5a5`, 343×36, 5 TEXT 노드 분리: Actor Name·Suffix1·Attendee Count·Suffix2·Status), **Feed Card**(`21732:3062` / key `1a348920b824461793300098c74f832f20f758b7`, 375×858, composed: Profile Card + Lounge Card + Media frame + Reaction Bar + Feed Addon Footer + Comment Item × 2). 4 컴포넌트 모두 instance probe 양/음 케이스 PASS. 후보 평가 evidence: CDS `Item` / `Lounge Card Addon Block` / `Messaging Reaction` reuse anatomy 호환 X로 reject. **알려진 한계**: Figma plugin API `addComponentProperty INSTANCE_SWAP` 호환 이슈로 Comment Item Right Slot + Feed Card 6개 slot prop이 BOOLEAN show/hide만 추가됨 (nested instance level swap으로 운용). **R8 hit area exception** 사용자 명시 승인 (Reaction Bar 22×22 / Comment Item 32×32 → implementation invisible padding 보정). Lounge Card 375 preflight PASS. 7회 peer review 사이클(분석 4 + plan 3) 거쳐 controller fallback. **Use site replacement BLOCKED** — CDS publish + 제품 파일 library update 사용자 액션 필요. 산출물: `.ai/pipeline/runs/20260508-131502_feed-screen-new-components/`, 스크린샷 `exports/2026-05-08_feed-cards-cds/feed-cards-group.png`.
...
### Changed
- **Lounge Update Item CDS composed component 생성 + 제품 사용처 교체 검증** — 기존 `Lounge Card`에 신규 variant를 추가하지 않고, 기존 카드를 nested exposed instance로 재사용하는 composed component `Lounge Update Item`을 생성. CDS library `H36eNEd6o7ZTv4R7VcyLf2`의 `Components > Composed > Lounge Cards > Main content` 아래 `21708:253`에 배치했고 component key는 `1dd4808f25b6577b8e6f9e3379295665175bb53c`. 구조는 `Lounge Card` + inline `.Reaction Bar` + `Reply Preview`이며, property reference/layout/text style/long text/responsive probes와 completed-work Claude review PASS. 초기에는 제품 파일 import가 publish/library update 전이라 blocked였지만, 후속 검증에서 `2026-05` Creator Case use-site가 remote `Lounge Update Item` 인스턴스 `28428:27223`, `28428:29052`, `28439:29132`로 교체된 것을 확인했고 원본 `25945:38066` 및 제안 source `28047:38198`은 제거됨. CDS/product Creator use-site 모두 Phosphor count 0. 사용자 리뷰 PASS.
- **연락처 친구 더보기 / 팔로우 관리 Figma 화면 V2 재작업** — 사용자 피드백 3건(`ㄱ/ㄴ/ㄷ 소팅`, CDS/유사 인스턴스 재사용, 직관적 레이블)을 반영해 `-lennyteam -play`로 기획부터 재진행. 기존 프레임 `28252:14022`, `28252:14125`를 제자리에서 V2로 교체했고, 연락처 초대 리스트는 `ㄱ`, `ㄴ`, `ㅁ`, `ㅈ`, `A` 그룹과 우측 index rail을 추가. v1의 `팔로우할 수 있는 연락처`/`초대할 연락처` 레이블을 제거하고 `Challify에 있는 친구`/`아직 가입하지 않은 연락처`로 교체. Contact 146개, Follow Management 96개 CDS/similar instances를 사용. Claude team-analysis/plan review PASS, screenshot QA 후 inherited tab red-dot과 반복 avatar 문제를 보정.
- **연락처 친구 더보기 / 팔로우 관리 Figma 화면 디자인** — 사용자 요청 `-play -director`로 `2026-05` Figma 파일의 현재 `팔로우 추천` 화면 옆에 `Contact Friends Screen`(`28252:14022`)과 `Follow Management Screen - Pure Management`(`28252:14125`) 신규 sibling frame을 생성. 연락처 화면은 `팔로우할 수 있는 연락처`와 `초대할 연락처`, 링크 초대를 분리하고, 팔로우 관리는 `팔로잉/팔로워` 탭과 관계 관리 액션 중심으로 구성해 추천/인기 feed와 분리. Metadata/screenshot QA 후 avatar 누락 2건을 initial avatar로 보정.
- **친구 찾기 / 팔로우 관리 디자인 PRD 작성** — `-director -play`로 팔로우 관리/친구 찾기 IA 결정을 디자인 PRD로 문서화. 산출물 `report/2026-05-06_friend-discovery-follow-management-design-prd.md`에 `Option A+D` 비대칭 내비게이션, 별도 `친구 찾기` screen, pure `팔로우 관리`, CDS `Recommendation Profile List` 선행 조건, metadata/source/follower count 규칙, 상태/접근성/측정/QA checklist를 정리. Claude team analysis/plan/completed PRD review 모두 PASS.
- **팔로우 관리 ↔ 친구 찾기 IA Team/Play 결정** — 사용자 옵션 1~4를 Team Mode(Product/Growth/Design/Data)로 검토하고 `-play`로 Claude peer review/plan-review를 3회 실행. 결론: `팔로우 관리`에는 우측 상단 `UserPlus`/`사람 추가` → `친구 찾기` 경로를 두되, `친구 찾기`에는 `팔로우 관리` 버튼과 팔로잉 preview를 노출하지 않는 비대칭 IA 채택. `친구 찾기`는 별도 screen으로 만들고, 기존 `팔로우 관리`의 `추천/인기` 탭은 제거. `Recommendation Profile List` composed pattern 생성을 선행 조건으로 기록.
- **라운지 즐겨찾기 필터 Team 리뷰** — 사용자 수정본 `2026-05` Figma `Lounge Screen`(`CS2ZhrORl4E1szQfTe2UvO/25429:16849`)의 `업데이트`/`즐겨찾기` chip 패턴을 Team Mode로 검토. 결론: 기존 group surface보다 우수하며, 개별 avatar favorite marker 대신 filter/view switch로 정의하는 방향 권장. 구현 전 Profile Card row dimension drift(90x108/90x88/100x82) 정규화 필요.
- **크리에이터 라운지 즐겨찾기 그룹 표현** — `Updates Area`(`t0SK7XaNqw8qIY3PpZw4s7/25972:52553`)에서 헤더-only `즐겨찾기 포함` 칩과 카드별 favorite star overlay를 제거하고, 첫 두 Profile Card를 옅은 `Favorite Lounge Group` surface로 묶어 하단 리스트 안에서 어떤 라운지가 즐겨찾기인지 식별되도록 조정. 비교를 위해 일반 라운지 카드 2개와 `일반` 라벨을 같은 row에 추가하고, 카드 폭을 88px로 조정해 `즐겨찾기 2개 + 일반 2개`가 한 화면에 보이도록 보정. `Creator Badge`/unread red-dot 메타포와 충돌하지 않도록 기존 Profile Card 인스턴스는 유지.
