...
**상황**: Tab Bar Badge 작업 중 Notification Badge(공용 TDS 컴포넌트)에 stroke 추가 제안

**증상**: 사용자가 "stroke 추가하면 어때?"라고 물었을 때 영향도 확인 없이 "좋습니다" 응답

**원인**: "동의 우선" 습관 — 사용자 제안에 즉시 동의하고 정합성 체크를 건너뜀

**영향**:
- 공용 컴포넌트 원본 수정 → 모든 사용처에 전파
- 인스턴스 오버라이드 → 원본 업데이트 시 깨짐 리스크
- 이미 존재하는 것을 무시하고 새로 만들라는 안내

**동일 세션 추가 사례**:
- 아이콘이 이미 인스턴스인데 "벡터 상태"라고 잘못 판단
- filled 아이콘을 이미 만들어뒀는데 "만들어주세요"라고 안내
- `get_figma_data` 결과를 읽지 않고 추측으로 응답

**재발방지**:
1. 수정 제안/동의 전 **사용처 확인** (이 컴포넌트가 공용인가?)
2. **원본 vs 인스턴스 구분** 명시 (원본 수정 = 전파, 오버라이드 = 깨짐)
3. **기존 존재 여부 확인** (`get_figma_data` 결과 먼저 읽기)
...
| 2026-03-15 | TDS 컴포넌트 | 공용 컴포넌트 수정 시 영향도 미확인 패턴 추가 |
